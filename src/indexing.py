...
class indexing:
    ...
    def add_new_documents(self, documents, collection_name="Agentic_retrieval"):
        ...
        try:
            ...
            batch_size = 5000  # Set a safe batch size, slightly below the limit
            ...
            for i in range(0, total_docs, batch_size):
                ...
                logger.info(f"⚡ Indexing batch {batch_num}/{total_batches} (Size: {len(batch)})")
                
                vector_store.add_documents(
                    documents=batch, 
                    # Ensure you generate a unique ID for every document
                    ids=[str(uuid6.uuid6()) for _ in batch] 
                )
            
            logger.info(f"✅ Successfully added {total_docs} new documents to '{collection_name}'")
            return True
