...
def markdown_to_html(text: str) -> str:
    """
    Converts basic markdown syntax (**bold**, *italic*, and bullet points) to HTML.
    """
    # Bold (**text** → <strong>text</strong>)
    text = re.sub(r"\*\*(.*?)\*\*", r"<strong>\1</strong>", text)

    # Bullet points (* item → <li>item</li>)
    def bullets_to_list(match):
        items = match.group(0).strip().split("\n")
        items = [f"<li>{item.lstrip('* ').strip()}</li>" for item in items if item.strip()]
        return "<ul>\n" + "\n".join(items) + "\n</ul>"

    text = re.sub(r"(?:^|\n)(\* .+(?:\n\* .+)*)", bullets_to_list, text)
    print(f"Text after markdown : {text}")
    return text
...
def parse_agent_response(response_dict):
    ...
    answer = markdown_to_html(response_dict.get('answer', 'Error: No answer found.'))
